...
const GroupRadioBtn: FC<IProps> = ({ user, checked, actualityChecked }) => {
  ...
  const activeLogic = (data: string[]) => {
    ...
    if (!!data.length) {
      !data.includes(id) &&
        localStorage.setItem("userActive", JSON.stringify([...data, id]));
  ...
  const disabledLogic = (data: string[]) => {
    ...
    if (!!data.length) {
      ...
      data.includes(id) &&
        localStorage.setItem("userActive", JSON.stringify(delUser));
    }
  };
